...
async def register_user(user: NewUser = Body(..., embed=True), engine: AIOEngine = EngineD):
    instance = UserModel(**user.dict(), hashed_password=get_password_hash(user.password))
...
async def login_user(
    ...
    user = await authenticate_user(engine, user_input.email, user_input.password.get_secret_value())
    if user is None:
