...
async def build_get_articles_query(
    ...
    if favorited is not None:
        favorited_user = await engine.find_one(UserModel, UserModel.username == favorited)
        if favorited_user is None:
            return None
        query &= {+ArticleModel.favorited_user_ids: {'$elemMatch': {'$eq': favorited_user.id}}}
...
async def add_article_to_favorite(engine: AIOEngine, user: UserModel, article: ArticleModel) -> ArticleModel:
...
async def remove_article_from_favorites(engine: AIOEngine, user: UserModel, article: ArticleModel) -> ArticleModel:
    favorited_set = {*article.favorited_user_ids} - {user.id}
    article.favorited_user_ids = tuple(favorited_set)
    await engine.save(article)
    return article
