...
class Article(BaseSchema):
    ...
    @classmethod
    def from_article_instance(cls, article: ArticleModel, user: UserModel | None = None) -> 'Article':
        if user is None:
            favorited = False
        else:
            favorited = user.id in article.favorited_user_ids

        return cls(favorited=favorited, favorites_count=len(article.favorited_user_ids), **article.dict())
...
class SingleArticleResponse(BaseSchema):
    ...
    @classmethod
    def from_article_instance(cls, article: ArticleModel, user: UserModel | None = None) -> 'SingleArticleResponse':
